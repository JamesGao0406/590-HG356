### This is the code write up for Part 2 (Logistic regression Model)
...
class logistic_one_class():
    ...
    def read(self):
        ...
        self.df.columns=['age','weight','is_adult']
    ...
    def model(self,x,p):##define logistic model 
    ...
    def visualization(self):
        ...
        x=self.df['age'].to_numpy()*self.x_std+self.x_mean
        y_model=self.model(self.df['age'].to_numpy(),self.parameter)*self.y_std+self.y_mean
        ax.plot(x, y_model, 'r-', label="logistic regression Model") 
        ax.legend()
        FS=18   #FONT SIZE
...
if __name__=='__main__':
    ...
    #linear regression
    lr=logistic_one_class(location)
    lr.read()
    lr.datasplit()
    lr.model_optimize()
    lr.visualization()