...
tol = 10**-10   			# exit after change in loss is less than 
...
def ANN(X,submatrices):
	...
	for i in range(0,len(layers)-1):
		index=2*i
		z=np.add(np.matmul(submatrices[index],z),submatrices[index+1])
		if i == len(layers)-2:
			#set linear function as the output layer
			a = z
...
def model(x,p):
	if(model_type=="linear" or model_type=="logistic" ):   
		...
		WB_submatrices=extract_submatrices(p)
		out=ANN(x,WB_submatrices)
	return  out  
...
def loss(p,index_2_use):
	...
	if model_type=='ANN':
		if regularization=='L1':
			training_loss=np.mean(errors**2.0)+GAMMA_L1*sum([abs(parameter) for parameter in p]) 
		elif regularization=='L2':
			training_loss=np.mean(errors**2.0)+GAMMA_L2*sum([parameter**2 for parameter in p]) 	
	else:
